...
이전 미션들과 달리 AWS를 이용해 배포를 해야했다.  
각자 하나의 EC2 인스턴스를 제공받을 수 있었고, 팀 별로 DB용 추가 인스턴스를 제공받았다.  
...
이번에 코멘트가 달려서 조금 더 자세히 공부해보기로 했다.  
...
- MySQL 8.0(InnoDB 사용시)의 경우 읽기 전용으로 알려진 트랜잭션의 경우 트랜잭션 ID를 설정하지 않기 때문에 조회 속도가 더 빨라진다.
...
DAO에 트랜잭션을 보장해보는건 어떻겠냐고 리뷰가 달려서 고민을 많이 했다.  
Service 계층에 이미 트랜잭션을 보장해주고 있기에 필요없지 않을까 생각했었다.  
DAO를 다른곳에서 사용하더라도 트랜잭션을 보장하기 위해(확장성 고려) 트랜잭션 애너테이션을 다는것도 괜찮은 것 같다.  