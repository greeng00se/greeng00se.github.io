---
title: '[우아GPT] 1. 프로토타입 만들기'
slug: 21
tags: [Woowachat, Retrospective]
---

### 4월 21일 금요일

레벨 2를 시작한 뒤 내가 학습에 대한 방향을 잃어버렸다는 생각이 들었다.  
레벨 3, 4때 나만의 강점을 가지고 싶어 고민을 많이 했다.  
단순히 스프링을 깊게 공부하는 건 효율이 많이 떨어진다고 생각했다.  
글쓰기 수상으로 받았던 쿠폰을 사용해 브라운에게 커피챗을 신청했고, 사이드 프로젝트를 해보라는 답을 받았다.  

나는 아이디어를 못내는 편인데 브라운이 아이디어까지 던져주셨다.  
`Chat-GPT 서비스를 크루들에게 제공하고, 해당 크루들이 질문한 내용을 공유할 수 있는 건 어떨지?`  

기술을 목적인 사이드 프로젝트를 진행하면 좋을 것 같다는 답변을 들었고, 혼자 아니면 페어할 수 있을 정도의 인원으로 진행하면 좋겠다고 하셨다.  
...
커피챗이 끝나고 집으로 돌아가는 길에 바로 도메인을 구매하려고 namecheap에서 적당한 도메인이 없을까 검색을 계속 했다.  
마치 어릴 때 했던 게임 닉네임 정하는 것 처럼 시간이 오래걸렸다.  
dev, io, chat 도메인이 후보였고 집가는 길에 결정만 하다가 구매하지 못했다.

### 말랑의 DM

집에가서 밥을 먹고 말랑이랑 DM 하다 프로젝트를 같이 하자는 이야기가 나왔다.  
...
프론트는 Nuxt3를 사용하기로 했고, Cloudflare Pages를 이용하여 배포했다.  

### GPT

무료 크레딧으로 사용하니 api limit이 있어 분당 3번까지 요청하지 못한다.  
...
아직 Tiptap이 제공하는 모든 기능을 자연스럽게 사용하지는 못하지만 CodeBlockLowlight 플러그인을 사용하여 코드블럭을 예쁘게 출력할 수 있었다.  
api 반환값 그대로 tiptap의 content에 설정했더니 코드블럭이 설정되지 않아서 백틱 3개를 `<pre><code>`로 replace 하고, `\n`은 `<br>`태그로 변환했다.  
...
Tiptap을 적용하니 다음과 같이 깔끔한 코드블럭을 볼 수 있었다.  

![woowachat](./woowa-chat.png)

### 폰트 및 favicon 적용

타이틀은 배달의 민족 도현체, 내용은 IBM Plex Sans를 사용했다.  
