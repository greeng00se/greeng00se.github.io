...
예전에도 미션을 진행하면서 [JdbcTemplate을 구현](./custom-jdbc-template)한 적이 있었는데, 그때와 비교하면 중복을 많이 제거해서 더욱 깔끔해진 모습이다.  
...
아래와 같은 구조로 미션을 진행했는데, ThreadLocal에 Connection 객체가 아닌, Connection 객체와 Trasnaction이 진행 중인지 확인할 수 있는 flag를 가지고 있는 클래스를 저장해서 사용하도록 했다.  
