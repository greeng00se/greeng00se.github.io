...
파일의 내용을 아래와 같이 수정하고, 이 때 setAttribute 부분은 적절하게 자신의 giscus 설정을 이용한다.  
...
유료 플랜이나 netlify를 사용하는 경우 크롤러를 따로 제공해주는 것 같다.  
...
다음 명령어를 이용해 .env와 config.json을 이용하여 크롤링을 한다.  
...
java나 kotlin의 경우 기본적으로 하이라이팅을 지원해주지 않는다.  
prism 설정을 아래와 같이 변경해준다.  
